...
export function createSelector(...input: any[]): Selector<any, any> {
    ...
    const memoizedProjector = memoize(function (...selectors1: any[]) {
        return projector.apply(null, selectors1);
    });

    const memoizedState = memoize(function (state: any) {
        const args1 = selectors.map(fn => fn(state));

        return memoizedProjector.memoized.apply(null, args1);
    });
